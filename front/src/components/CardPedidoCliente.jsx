...
function CardPedidoCliente({ id, user_id }) {
  ...
  const fetchPedidoVianda = async () => {
    await axios.get(`${SERVER}porPedidoVianda/${id}`).then((res) => {
  ...
  const pedidoHoy = pedidoVianda.filter(
    (pedido) => pedido.fechaEntrega === fecha
  );
  // console.log(pedidoHoy);
        ...
        <div key={index} className="">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 items-center mb-1 p-1 bg-purple-100">
            <div className="col-span-2 flex items-center gap-4">
              <img
                src={`http://localhost:8000${pedido.vianda.urlFoto}`}
                className="w-14 h-14 object-cover rounded-xl"
              />
              {/* <h3>pedidoVianda id: {pedido.id}</h3> */}
              <div className="col flex items-center gap-4">
                <h3 className="font-bold text-lg p-2">
                  {pedido.vianda.nombre}
                </h3>
                <p className="text-gray-600">Cantidad {pedido.cantidad}</p>
              </div>
              <div>
                <h3 className="font-bold p-2 text-indigo-600 gap-4">
                  {pedido.lugar_entrega.nombreLugar}
                </h3>

                <p className="text-gray-500">
                  {pedido.lugar_entrega.calle} {pedido.lugar_entrega.nroCalle}{" "}
                </p>
              </div>
            </div>
            <div>
            ...
            <div className="col-span-1 flex items-center gap-4">
              ...
              <button
                ...
                onClick={() => handleOpenPopup(pedido.id)}
              >
                <RiTicketLine
                  className={`text-3xl p-1 box-content rounded-xl text-green-800`}
                />
              </button>
            </div>
          </div>
      ...
      {showPopup && (
        <div className="popup">
          <div className="popup_inner">
            ...
            <button
              className="bg-gray-500 p-1 text-white w-48"
